...
#
# runs sequentially, not in parallel -- future::multisession kept hanging
# indefinitely after the progress bar hit 100% (the classic macOS "R would
# like to accept incoming network connections" firewall prompt, easy to miss
# behind another window, and future gives no timeout or visible error while
# it waits). boot_one() already sets its own seed per-iteration
# (BOOT_SEED_INTERVENTION + b), so sequential vs. parallel execution changes
# nothing about the results, only the wall-clock time. slower, but it can't
# silently hang the same way.
...
boot_list <- purrr::map(seq_len(N_BOOT_INTERVENTION), boot_one, .progress = TRUE)
